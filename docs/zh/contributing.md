...
- `@panzhiyue/vuepress-theme-knowledge`: 默认主题。
...
- `vuepress-vite`: 是 `@vuepress/cli` + `@vuepress/bundler-vite` + `@panzhiyue/vuepress-theme-knowledge` 的封装。如果用户想使用 默认主题 + Vite ，仅安装这个 Package 就可以了。

- `vuepress-webpack`: 是 `@vuepress/cli` + `@vuepress/bundler-webpack` + `@panzhiyue/vuepress-theme-knowledge` 的封装。如果用户想使用 默认主题 + Webpack ，仅安装这个 Package 就可以了。
